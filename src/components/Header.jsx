...
const Header = () => {
  ...
  const handledashbord = () => {
    ...
    if (role === "admin") {
      ...
      toast.error("sorry! you cannot access Dashbord route before becoming admin (select role as admin)")
    }
  }
  ...
  return (<>
    <header className='sticky w-screen top-0 z-50 bg-[#101828] py-4 px-5 text-gray-300 flex flex-row justify-between md:justify-around'>
      ...
      <div className='flex flex-row gap-2 items-center justify-center'>
        <button onClick={handledashbord} className="bg-green-500 text-white hover:bg-green-600 text-black cursor-pointer rounded-lg hover:bg-black hover:bg-gray-200 transition duration-300 py-1.5 px-1.5  md:py-2.5 md:px-2.5 font-semibold text-md md:text-lg">Dashbord</button>
        {
          token ? <button onClick={clearToken} className='bg-red-500 hover:bg-red-600 text-white  cursor-pointer rounded-md transition transition duration-300 duration-300 py-1.5 px-1.5 md:py-2.5 md:px-2.5 font-semibold  text-md md:text-lg'>Logout</button>
    ...
    <div className='md:hidden flex flex-row justify-around fixed bg-[#101828] bottom-0 left-0 m-auto w-full z-20 text gap-3 items-center justify-center '>
      <ul className='flex justify-around w-full flex-row gap-3 py-6' >
        <li><NavLink to={"/"} className={({ isActive }) =>
          isActive ?
            "font-semibold text-xl md:text-2xl font-extrabold text-indigo-400 hover:text-blue-300 transition duration-200 " :
            "font-semibold text-xl md:text-2xl font-extrabold text-gray-400"
        } >Home🏠</NavLink></li>
        <li><NavLink to={"/blogs"} className={({ isActive }) =>
          isActive ?
            "font-semibold text-xl md:text-2xl font-extrabold text-indigo-400 hover:text-blue-300 transition duration-200 " :
